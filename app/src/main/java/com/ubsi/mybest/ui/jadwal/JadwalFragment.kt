package com.ubsi.mybest.ui.jadwal
...
import com.ubsi.mybest.databinding.FragmentJadwalBinding
...
class JadwalFragment : Fragment() {
    ...
    private fun populateScheduleList(schedules: List<ScheduleEntity>) {
        ...
        schedules.forEachIndexed { index, schedule ->
            ...
            val cardView = createScheduleCard(schedule, headerColor)
     ...
     * - Red (#E74C3C): Future day (belum mulai)
     * - Green (#27AE60): Today and will start soon
     * - Gray (#95A5A6): Today but time has passed
     */
    private fun getScheduleColor(schedule: ScheduleEntity): String {
        ...
        if (endTimeParts.size == 2) {
            ...
            return if (currentTotalMinutes >= endTotalMinutes) {
                "#95A5A6" // Time has passed - Gray
            } else {
                "#27AE60" // Today and will start/ongoing - Green
            }
        }
    ...
    private fun createScheduleCard(schedule: ScheduleEntity, headerColor: String): CardView {
        val inflater = LayoutInflater.from(requireContext())
        val cardView = inflater.inflate(R.layout.item_schedule_card, binding.scheduleListContainer, false) as CardView
        
        cardView.findViewById<View>(R.id.headerLayout)?.setBackgroundColor(android.graphics.Color.parseColor(headerColor))
        ...
        cardView.setOnClickListener {
            com.ubsi.mybest.ui.presensi.PresensiActivity.start(requireContext(), schedule.id)
        }
